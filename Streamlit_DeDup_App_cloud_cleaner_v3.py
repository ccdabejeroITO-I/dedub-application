...
def _styler_map_safe(styler, func, subset=None):
...
def _build_preview_styler(df: pd.DataFrame):
    ...
    if "DedupType" in df.columns:
        styler = _styler_map_safe(styler, _dedup_type_badge_css, subset=["DedupType"])
    if "Confidence" in df.columns:
        styler = _styler_map_safe(styler, _confidence_badge_css, subset=["Confidence"])
    return styler
